...
namespace ldso {
     ...
     *
     * loop closing is running in a single thread, receiving new keyframes from the front end. It will seprate all keyframes into several "consistent groups", 
     * and if the newest keyframe seems to be consistent with a previous group, we say a loop closure is detected. And once we find a loop, we will check the 
     * sim3 and correct it using a global bundle adjustment.
     * 
     */
    class LoopClosing {
    public:
